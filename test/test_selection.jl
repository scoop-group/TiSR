...
@testset "non_dominated_sort" begin
    ...
    inds = non_dominated_sort(data; n_select=100, first_front=false)
    ...
    inds = non_dominated_sort(data; n_select=100, first_front=true)
    ...
    inds = non_dominated_sort(data; n_select=40, first_front=false)
    @test length(unique(inds)) == 40
    @test all(i < 41 for i in inds)
    @test all(i in eachindex(x_data) for i in inds)

    inds = non_dominated_sort(data; n_select=35, first_front=false)
    @test length(unique(inds)) == 35
    @test all(i in inds for i in 1:30)
    @test all(i in eachindex(x_data) for i in inds)

    # ----------------------------------------------------------------------------------------------
    x_data = repeat(1.:10., 10)                                                                      # generate a grid of points
    y_data = sort(x_data)
    data = [x_data, y_data]
    ...
    inds = non_dominated_sort(data; n_select=num, first_front=false)
    ...
    @test length(non_dominated_sort(data; n_select=25, first_front=false)) == 25                      # this tests whether the function can handle if one variable is the same for all points -> there was a division by 0 error in the crowing distance before
end
...
@testset "crowding_distance_selection" begin
    ...
    num_select = 20
    inds = crowding_distance_selection(arr, all_inds, num_select)
    @test length(unique(inds)) == num_select
    @test all(i in all_inds for i in inds)

    dist = crowding_distance(arr)
